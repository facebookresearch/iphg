...
class SpatioTemporalAdam(torch.optim.Optimizer):
    def __init__(
        self,
        params,
        lr=0.1,
        ...
        super(SpatioTemporalAdam, self).__init__(params, defaults)
    ...
    def __setstate__(self, state):
        super(SpatioTemporalAdam, self).__setstate__(state)
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if normals is not None:
                    n_repeat = (grad.numel() // grad.shape[-1]) // (
                        normals.numel() // normals.shape[-1]
                    )
                    normals_reshaped = normals.repeat(1, n_repeat, 1).reshape(-1, 3)

                if self.prefilter != "off" and M is not None and normals is not None:
                    grad = self.filter(
                        grad, filt_type=self.prefilter, normals=normals_reshaped, M=M
                    )

                g1.mul_(b1).add_(grad, alpha=1 - b1)
                g2.mul_(b2).add_(grad.square(), alpha=1 - b2)

                if self.postfilter != "off" and M is not None and normals is not None:
                    g1 = self.filter(
                        g1, filt_type=self.postfilter, normals=normals_reshaped, M=M
                    )
